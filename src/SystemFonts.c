...
/* CavFX: cc_* FreeType wrappers are defined once globally near the top of this file. */
...
static void SysFonts_Add(const cc_string* path, FT_Face face, int index, char type, const char* defStyle) {
	...
	if (!face || !face->family_name || !face->family_name[0]) return;
	if (!(face->face_flags & FT_FACE_FLAG_SCALABLE)) return;
	...
	if (style.length) {
		String_Format3(&key, "%c %c %r", face->family_name, face->style_name ? face->style_name : "", &type);
	} else {
